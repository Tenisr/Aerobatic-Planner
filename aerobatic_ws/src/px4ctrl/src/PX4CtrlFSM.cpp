...
PX4CtrlFSM::PX4CtrlFSM(Parameter_t &param_, LinearControl &controller_, SO3Control &_controller_) : param(param_), controller(controller_), controller_(_controller_)	//好歹也是把SO3硬塞进来了
{
	state = MANUAL_CTRL;
	hover_pose.setZero();
}
...
void PX4CtrlFSM::process()
{
	...
	if (state == AUTO_HOVER || state == CMD_CTRL)
	{
		...
		controller_.estimateThrustModel(imu_data.a,param);
...
Desired_State_t PX4CtrlFSM::get_hover_des()
{
	...
	des.dir = Eigen::Vector3d(cos(des.yaw), sin(des.yaw), 0.0);
	des.dir_dot = Eigen::Vector3d::Zero();
...
Desired_State_t PX4CtrlFSM::get_cmd_des()
{
	...
	des.dir = cmd_data.dir;
  	des.dir_dot = cmd_data.dir_dot;
