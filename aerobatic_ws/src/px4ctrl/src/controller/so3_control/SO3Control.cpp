...
SO3Control::SO3Control(Parameter_t param) : param_(param)
    , mass_(0.98), g_(9.81)
{
  acc_.setZero();
  omega_.setZero();
...
quadrotor_msgs::Px4ctrlDebug SO3Control::calculateControl(const Eigen::Vector3d &des_pos,
                                  ...
                                  const Imu_Data_t &imu,const Odom_Data_t &odom)
{
  ...
  kx << param_.gain.Kp0, param_.gain.Kp1, param_.gain.Kp2;
  kv << param_.gain.Kv0, param_.gain.Kv1, param_.gain.Kv2;
  kR << param_.gain.KAngR, param_.gain.KAngP, param_.gain.KAngY;

  // x acceleration
  // 截断只是为了辅助修正
  double x_pos_error = std::isnan(des_pos(0)) ? 0.0 : std::max(std::min(des_pos(0) - pos_(0), 2.0), -2.0);  //那么这里的限制应该单独作为一个参数使用的
  double x_vel_error = std::max(std::min((des_vel(0) + kx(0) * x_pos_error) - vel_(0), 2.0), -2.0);
  // double x_pos_error = std::isnan(des_pos(0)) ? 0.0 : (des_pos(0) - pos_(0));
  // double x_vel_error = (des_vel(0) + kx(0) * x_pos_error) - vel_(0);
  acc_error(0) = kv(0) * x_vel_error;

  // y acceleration
  double y_pos_error = std::isnan(des_pos(1)) ? 0.0 : std::max(std::min(des_pos(1) - pos_(1), 2.0), -2.0);
  double y_vel_error = std::max(std::min((des_vel(1) + kx(1) * y_pos_error) - vel_(1), 2.0), -2.0);
  // double y_pos_error = std::isnan(des_pos(1)) ? 0.0 : (des_pos(1) - pos_(1));
  // double y_vel_error = (des_vel(1) + kx(1) * y_pos_error) - vel_(1);
  acc_error(1) = kv(1) * y_vel_error;

  // z acceleration
  double z_pos_error = std::isnan(des_pos(2)) ? 0.0 : std::max(std::min(des_pos(2) - pos_(2), 2.0), -2.0);
  double z_vel_error = std::max(std::min((des_vel(2) + kx(2) * z_pos_error) - vel_(2), 2.0), -2.0);
  // double z_pos_error = std::isnan(des_pos(2)) ? 0.0 : (des_pos(2) - pos_(2));
  // double z_vel_error = (des_vel(2) + kx(2) * z_pos_error) - vel_(2);
  acc_error(2) = kv(2) * z_vel_error;

  // std::cout << "acc_error = " << acc_error.transpose() << std::endl;
  force_ = mass_ * g_ * Eigen::Vector3d(0, 0, 1);
  force_ += mass_ * (des_acc + acc_error);    //命名问题acc_error实际为闭环反馈力，kp与kv实际产生的是物理力反馈，但是原本的质量为1Kg左右，不乘以mass反馈力就会太大了
  ...
  debug_msg_.des_a_x = force_.x() / mass_;
  debug_msg_.des_a_y = force_.y() / mass_;
  debug_msg_.des_a_z = force_.z() / mass_;

  debug_msg_.des_q_x = orientation_.x();
  debug_msg_.des_q_y = orientation_.y();
  debug_msg_.des_q_z = orientation_.z();
  debug_msg_.des_q_w = orientation_.w();
                         ...
                         Eigen::Vector3d &omg)
{
  ...
  // 计算姿态q
  tilt_den = sqrt(2.0 * std::max(0.0, 1.0 + z2));
  if (tilt_den < 1.0e-5) tilt_den = 1.0e-5;
  tilt0 = 0.5 * tilt_den;
  tilt1 = -z1 / tilt_den;
  tilt2 = z0 / tilt_den;
  // if (z2 + 1.0 < 1.0e-5)
...
const double SO3Control::computeDesiredCollectiveThrustSignal(
    const Eigen::Vector3d &des_acc)
{
...
void 
SO3Control::resetThrustMapping(void)
{
  thr2acc_ = param_.gra / param_.thr_map.hover_percentage;
  P_ = 1e6;
}

const bool SO3Control::estimateThrustModel(
