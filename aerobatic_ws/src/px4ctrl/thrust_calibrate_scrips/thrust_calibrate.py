...
# 标定流程说明
# 让无人机悬停在不同油门下（可通过手动或脚本逐步改变推力）
# 发布第一次 /traj_start_trigger → 开始采集
# 采集一段时间后，发布第二次 /traj_start_trigger → 停止并保存
# 用 data.csv 中的 (cmd, voltage) 数据对公式：
# $$F = K_1 \cdot V^{K_2} \cdot (K_3 \cdot u^2 + (1-K_3) \cdot u)$$

# 进行曲线拟合，求解 K1、K2、K3，填入 ctrl_param_fpv.yaml。